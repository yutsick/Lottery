import { accountActions } from '../store/action-names';

export default function() {

        let currentActiveNavItem = null;
		let $slideOutNavigationBtn = $('.js-toggle-account');
		let $overlay = $('.js-top__overlay');
		let $slideOutNavigation = $('.js-account-navigation');
        ...
        let overlayBtnRightClassName = 'top__overlay-btn--right';
		...
		const dispatchEvents = (showNavigation) => {
			window.ML.store.account.dispatch({ type: accountActions.SHOWACCOUNT, payload: showNavigation });				
		}
		...
		const showOverlay = (show) => {
			if(show) {
                ...
                $overlayBtn.addClass(overlayBtnRightClassName);
				...
				setTimeout(() => {
                    ...
                    $overlayBtn.removeClass(overlayBtnRightClassName);
				}, 300);
		...
		window.ML.store.account.subscribe(() => {
			updateSlideOutNavigation(navigationState());
		});
